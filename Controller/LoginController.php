...
if ($_POST) {
    ...
    if (!empty ($_POST['username']) && !empty ($_POST['password'])) {
        ...
        if ($result->num_rows && $userLoginArray[0]["UserStatus"] == "Active" && ($userLoginArray[0]["Ban"] == 0 ||
            ...
            if (
                $userLoginArray[0]["RoleId"] != \Model\Role::ADMIN &&
                $userLoginArray[0]["RoleId"] != \Model\Role::MODERATOR &&
                $userLoginArray[0]["RoleId"] != \Model\Role::LEGEND_USER
            ) {
                if($countComments > 100 && $countComments < 200){
                    $user->updateRole(\Model\Role::POWER_USER, $userLoginArray[0]["UserId"]);
                }elseif($countComments > 200 && $countComments < 400){
                    $user->updateRole(\Model\Role::ELITE_USER, $userLoginArray[0]["UserId"]);
                }elseif($countComments > 400){
                    $user->updateRole(\Model\Role::LEGEND_USER, $userLoginArray[0]["UserId"]);
                }
            }
